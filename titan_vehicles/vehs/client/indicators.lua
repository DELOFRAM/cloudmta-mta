...
----------------
	...
	local INDICATOR_AUTOSWITCH_OFF_THRESHOLD = 0   -- A value in degrees ranging (0, 90) preferibly far from the limits.
	...
	* destroyIndicatorState
	...
	local function destroyIndicatorState ( state )
		...
		-- if getVehicleOccupant ( state.vehicle ) == localPlayer then
			...
			setElementData(state.vehicle, "indicator", false, false)
		-- end
	end
		...
		if state.nextChange <= state.timeElapsed then
			...
			if state.currentState then
				...
				if playerVehicle == state.vehicle then
					...
					if(getElementData(playerVehicle, "i:left")) then 
						setElementData(playerVehicle, "indicator", 1, false)
					elseif(getElementData(playerVehicle, "i:right")) then
						setElementData(playerVehicle, "indicator", 2, false)
					end
				end
				...
				if playerVehicle == state.vehicle then
					...
					setElementData(playerVehicle, "indicator", false, false)
				end	
			end
	...
	* onClientPreRender
	...
	if getElementData(localPlayer, "enteringVehicle") or getElementData(localPlayer, "exitingVehicle") then return end
		if (keyPressed == ",") or (keyPressed == "[") then
			local veh = getPedOccupiedVehicle(localPlayer)
			if(isElement(veh) and getVehicleOccupant(veh) == localPlayer) then
				if(getElementData(veh, "i:right")) then
					setElementData(veh, "i:right", false)
				end
				switchIndicatorState('left')
			end
		elseif (keyPressed == ".") or (keyPressed == "]") then
			local veh = getPedOccupiedVehicle(localPlayer)
			if(isElement(veh) and getVehicleOccupant(veh) == localPlayer) then
				if(getElementData(veh, "i:left")) then
