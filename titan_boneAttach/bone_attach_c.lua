...
function putAttachedElementsOnBones()
	for element,ped in pairs(attached_ped) do
		if not isElement(element) then
			return clearAttachmentData(element)
		elseif isElement(ped) and isElementStreamedIn(ped) then
			...
			if tonumber(objx) and tonumber(objy) and tonumber(objz) and isElement(element) then
				objx = tonumber(objx)
				objy = tonumber(objy)
				objz = tonumber(objz)
				offrx = tonumber(offrx)
				offry = tonumber(offry)
				offrz = tonumber(offrz)
