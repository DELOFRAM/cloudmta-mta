...
Class.HUD.bar.images[1] = "punchbag"
...
function showBarPunchBag()
	local width = math.max(math.min(panchBag.Processing or 0,Class.HUD.bar.Full[1].W),0)
	...
	local width = math.max( math.min(Class.HUD.bar.Full[1].W-panchBag.Fatigue or 0, Class.HUD.bar.Full[1].W),0) 
